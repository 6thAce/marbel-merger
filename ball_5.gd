...
func merge_with(other_ball: RigidBody2D) -> void:
	...
	# --- SPECIFIC CHANGE: Only set level to -1 if the ball is NOT the final ball (Level 5) ---
	# We use main.ball_scenes.size() - 1 to check for the index of the last ball (Level 5).
	if level < main.ball_scenes.size() - 1:
